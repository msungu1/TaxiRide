...
class _RideWaitingScreenState extends State<RideWaitingScreen> {
  ...
  void _listenToRideUpdates() {
    _rideSubscription = SocketService.instance.rideUpdates.listen((event) {
      ...
      debugPrint("📡 Waiting Screen Socket Frame Received: $event");

      if (!mounted) return;

      // ---------------- 1. TRIP CANCELLED / REJECTED ----------------
      if (type == 'trip_cancelled' ||
          type == 'ride_rejected' ||
          type == 'ride_declined' ||
          status == 'cancelled' ||
      ...
      // ---------------- 3. DRIVER ASSIGNED (THE ONLY CLEAN HANDOFF ENTRY) ----------------
      if (type == 'ride_accepted_by_driver' || status == 'accepted' || status == 'assigned') {

        // Stop everything on this screen immediately!
        _cleanupTimersAndStreams();

        if (_hasNavigated) return;
        _hasNavigated = true;

        debugPrint("✅ Driver confirmed assignment. Transitioning and destroying waiting context.");
        ...
        // Small delay for natural animation visual tracking handoff
        Future.delayed(const Duration(milliseconds: 500), () {
          ...
          Navigator.pushReplacement(
            ...
            MaterialPageRoute(
              builder: (_) => PassengerRideDetailScreen(
                rideData: event, // Pass the fresh backend event data with driver info nested inside
              ),
            ),
          );
        });
  ...
  void _startPolling() {
    _pollingTimer = Timer.periodic(const Duration(seconds: 6), (timer) async {
      try {
        final response = await http.get(
          Uri.parse('https://sizemoretaxi-itpj.onrender.com/api/trips/${widget.tripId}'),
          headers: {'Accept': 'application/json'},
        );
        ...
        switch (status) {
          case 'requested':
            setState(() { statusMessage = "Searching for available drivers..."; });
            break;
          case 'accepted':
            setState(() { statusMessage = "Driver accepted your request..."; });
            break;
          case 'assigned':
          case 'in_progress':
          case 'ongoing':
          // Fallback safety route if sockets missed a packet drop frame
            _cleanupTimersAndStreams();
            if (!_hasNavigated) {
              _hasNavigated = true;
              Navigator.pushReplacement(
                context,
                MaterialPageRoute(builder: (_) => PassengerRideDetailScreen(rideData: trip)),
              );
            }
            break;
          case 'cancelled':
        ...
        debugPrint("Polling connection oversight error: $e");
      }
    });
  }

  void _cleanupTimersAndStreams() {
    _pollingTimer?.cancel();
    _rideSubscription?.cancel();
    _pollingTimer = null;
  ...
  Future<void> _cancelRide() async {
    ...
    try {
      ...
      await http.post(
        Uri.parse('https://sizemoretaxi-itpj.onrender.com/api/trips/cancel-rider'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
      ...
      debugPrint("Cancel request dispatch failure: $e");
    }
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 28),
                      const Text("Cancel Ride?", style: TextStyle(fontSize: 26, fontWeight: FontWeight.bold, color: Colors.black)),
                      const SizedBox(height: 8),
                      Text("Please select a reason for cancellation.", style: TextStyle(fontSize: 15, color: Colors.grey.shade600)),
                      const SizedBox(height: 24),
                      Column(
                        children: cancelReasons.map((reason) {
                          final isSelected = selectedReason == reason;
                              ...
                              child: Row(
                                children: [
                                  Expanded(
                                    child: Text(
                                      reason,
                                      style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: isSelected ? Colors.white : Colors.black),
                                    ),
                                  ),
                                  AnimatedSwitcher(
                                    duration: const Duration(milliseconds: 200),
                                    child: isSelected
                                        ? const Icon(Icons.check_circle, color: Colors.white, key: ValueKey("selected"))
                                        : const SizedBox.shrink(),
                                  ),
                                ],
                              ),
                      ...
                      if (selectedReason == "Other") ...[
                        const SizedBox(height: 10),
                        Container(
                          decoration: BoxDecoration(color: const Color(0xFFF5F5F5), borderRadius: BorderRadius.circular(18), border: Border.all(color: Colors.grey.shade300)),
                          child: TextField(
                            controller: _cancelReasonController,
                            maxLines: 4,
                        ...
                        children: [
                          Expanded(
                            child: OutlinedButton(
                              ...
                              child: const Text("Keep Ride", style: TextStyle(color: Colors.black87, fontWeight: FontWeight.w600)),
                            ),
                          ),
                          const SizedBox(width: 14),
                          Expanded(
                            child: ElevatedButton(
                              ...
                              onPressed: () async {
                                if (selectedReason == null) return;
                                if (selectedReason != "Other") {
                                  _cancelReasonController.text = selectedReason!;
                                }
                                Navigator.pop(context);
                                await _cancelRide();
                              },
            ...
            child: Column(
              ...
              children: [
                ...
                TextButton(
                  ...
                  child: Text(
                    "Cancel Request",
                    style: GoogleFonts.notoSans(color: Colors.redAccent, fontWeight: FontWeight.bold),
                  ),
                ),
              ],
            ),
