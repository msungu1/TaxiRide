...
class _DispatchManagementScreenState extends State<DispatchManagementScreen> {
  ...
  Future<void> _fetchAvailableDrivers() async {
    try {
      ...
      final List<dynamic> allDriversData = await AdminApiService.getAvailableDrivers(widget.tripId);

      List<UserModel> fetchedDrivers = allDriversData.map((d) => UserModel.fromJson(d)).toList();
      ...
      _updateMarkers();
      // After fetching, try to fit the map bounds
      _fitMapToMarkers();
  ...
  void _updateMarkers() {
    ...
    // 1. Pickup Marker
    if (widget.rideData['pickupLocation'] != null) {
      newMarkers.add(Marker(
        markerId: const MarkerId('pickup'),
        position: LatLng(
          widget.rideData['pickupLocation']['lat']?.toDouble() ?? 0.0,
          widget.rideData['pickupLocation']['lng']?.toDouble() ?? 0.0,
        ),
      ...
      if (driver.currentLocation != null) {
        newMarkers.add(Marker(
          ...
          infoWindow: InfoWindow(
              title: driver.name,
// The ?? false means: "If isOnline is null, treat it as false"
  ...
  // Automatically adjusts the camera to see all markers
  ...
  String _calculateDistance(Map<String, dynamic>? driverLoc, Map<String, dynamic>? pickupLoc) {
    if (driverLoc == null || pickupLoc == null) return "Unknown distance";
    try {
          ...
          Expanded(
            ...
            child: GoogleMap(
              ...
              onMapCreated: (controller) {
                ...
                _fitMapToMarkers(); // Fit markers once map is ready
              },
            ),
          ),
          Expanded(
              ...
              child: Column(
                children: [
                  const Padding(
                    ...
                    child: Text("Available Drivers", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                  ),
                  Expanded(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            ...
                            subtitle: Text("$distance • ${driver.carType}", style: const TextStyle(color: Colors.green)),
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(onPressed: () => Navigator.pop(context, true), style: ElevatedButton.styleFrom(backgroundColor: Colors.green), child: const Text("Confirm")),
        ],
      ),
    );
